...
def upload_to_reducto(file_path: Path) -> str:
    with step_timer(
        ...
        step.set(file_id=file_id)
        return file_id


def submit_parse_job(file_id: str) -> str:
    with step_timer(logger, "parse.submit_parse_job", file_id=file_id) as step:
        response = requests.post(
...
def wait_for_job(job_id: str, timeout: int = 300) -> dict:
    with step_timer(logger, "parse.wait_for_job", job_id=job_id, timeout_s=timeout) as step:
        start = time.time()
        polls = 0
        while True:
            polls += 1
            ...
            if status in ("Completed", "Failed"):
                step.set(job_status=status, polls=polls)
                return job
...
def _extract_chunks(parse_job: dict) -> list[dict]:
    ...
    if inner.get("type") == "url":
        try:
            return requests.get(inner["url"]).json()
        except Exception:
            return []
...
def parse_manifest(manifest_path: Path) -> dict:
    with step_timer(logger, "parse.manifest", manifest_path=str(manifest_path)) as overall:
        _check_env()
        ...
        with step_timer(logger, "parse.phase_submit", file_count=len(files)):
            for entry in files:
                ...
                with step_timer(
                    ...
                    file_step.set(file_id=file_id, parse_job_id=parse_job_id, extract_job_id=extract_job_id)
        ...
        if pending_jobs:
            with step_timer(
                ...
                parse_timeout_s=PARSE_JOB_TIMEOUT_S,
                extract_timeout_s=EXTRACT_JOB_TIMEOUT_S,
            ):
                    ...
                    try:
                        with step_timer(
                            ...
                            file_step.set(
                                ...
                                extract_status="ok" if not extract_error else "error",
                            )
                    except Exception as exc:
                        log_event(
                            logger,
                            "parse.file_finalize_error",
                            level=40,
                            file_id=file_id,
                            error=str(exc),
                        )
