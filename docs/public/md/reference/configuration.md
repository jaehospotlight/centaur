...
| `overlays.sources` | Ordered repo-cache-backed overlay repos for tools, workflows, and skills. |
...
| `TOOL_DIRS`, `PLUGINS_DIR` | Chart-rendered from `overlays.sources[*].toolsSubdir`; fallback to `PLUGINS_DIR`. | Tool discovery paths. |
| `WORKFLOW_DIRS` | Chart-rendered from `overlays.sources[*].workflowsSubdir`. | Workflow discovery paths. |
...
| `KUBERNETES_WORKFLOW_DIRS` | Chart-rendered from `overlays.sources[*].workflowsSubdir` using the sandbox repo-cache mount prefix. | Workflow-host sandbox discovery paths. |
...
| `CENTAUR_SKILL_DIRS` | Chart-rendered from `overlays.sources[*].skillsSubdir` through `SESSION_SANDBOX_EXTRA_ENV`. | Ordered skill directories copied into the agent workspace. |
