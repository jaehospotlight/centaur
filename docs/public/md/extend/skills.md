...
overlay deployment, they must exist under the configured `skillsSubdir` in the
sandbox repo checkout, for example
`/home/agent/github/your-org/centaur-overlay/.agents/skills`. The sandbox
entrypoint copies those skills into the agent workspace during startup.
