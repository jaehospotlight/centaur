...
| `CENTAUR_OVERLAY_DIR` | `overlay.repo`, `overlay.mountPath`, or `overlay.systemPrompt`. | Mounted overlay root for tools, workflows, prompts, migrations, and skills. Repo-cache mode uses the live checkout path. |
| `CENTAUR_OVERLAY_REPO` | `overlay.repo`. | Owner/repo slug for live repo-cache overlays mounted into sandboxes. |
