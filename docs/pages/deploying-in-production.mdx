...
- `OPENAI_CODEX_CLIENT_ID`: the Codex CLI OAuth client id collected by
  `centaur secrets collect`. The broker resolves it through your secrets
...
`~/.codex/auth.json`, prompts for any missing masked values, and writes the
matching secret items to the selected backend.
...
- `CLAUDE_CODE_CLIENT_ID`: the Claude Code CLI OAuth client id collected
  by `centaur secrets collect`. The broker resolves it through your
  secrets backend alongside the refresh-token blob.
...
reads the credentials file or macOS keychain item, prompts for any missing
masked values, and writes the matching secret items to the selected backend.
