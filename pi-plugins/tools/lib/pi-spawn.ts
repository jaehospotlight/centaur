/**
 * Core sub-agent spawning utility.
 *
 * Extracts the spawn-parse-collect loop into a reusable function.
 * Dedicated tools (finder, oracle, Task) call piSpawn() with their
 * own config. Supports JSON/print mode and RPC mode (for follow-up).
 */
...
export interface PiSpawnConfig {
   ...
   * Switches to RPC mode: the follow-up is queued eagerly at startup
   * and delivered after the first end_turn. Process is killed after
   * the expected number of turns completes.
   */
  followUp?: string;
}
...
function writeTempPrompt(prompt: string): { dir: string; filePath: string } {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "pi-subagent-"));
  const filePath = path.join(dir, "prompt-subagent.md");
...
export async function piSpawn(config: PiSpawnConfig): Promise<PiSpawnResult> {
  ...
  try {
    // Write interpolated system prompt to temp file
    if (config.systemPromptBody?.trim()) {
      const interpolated = interpolatePromptVars(config.systemPromptBody, config.cwd, {
        sessionId: config.sessionId,
        repo: config.repo,
      });
      const tmp = writeTempPrompt(interpolated);
      tmpDir = tmp.dir;
      tmpFile = tmp.filePath;
      args.push("--append-system-prompt", tmpFile);
    }

    // In print mode, task is a CLI arg; in RPC mode, sent via stdin
    if (!useRpc) {
    ...
    if (config.extensionTools) {
      env.PI_INCLUDE_TOOLS = config.extensionTools.join(",");
    }

    let wasAborted = false;
    const debug = process.env.PI_SPAWN_DEBUG
    ...
    const exitCode = await new Promise<number>((resolve) => {
      ...
      // RPC mode: send prompt and follow-up eagerly via stdin
      ...
      const processLine = (line: string) => {
        ...
        // Skip RPC protocol ack responses
        ...
        if (event.type === "message_end" && event.message) {
          ...
          if (msg.role === "assistant") {
            ...
            // RPC: kill after expected number of end_turns
            if (useRpc && isTurnEnd) {
              endTurnCount++;
    ...
    // RPC processes killed intentionally get normalized exit code
    if (useRpc && result.exitCode !== 0) {
